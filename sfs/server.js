...
function createSfsServer() {
  return net.createServer((socket) => {
    ...
    // Prevent socket timeout - SmartFoxServer connections should remain open indefinitely
    socket.setTimeout(0); // Disable automatic timeout
    
    // Set keep-alive to ensure connection stays alive
    socket.setKeepAlive(true, 30000); // Send keep-alive after 30 seconds of inactivity
    ...
    socket.on('data', (data) => {
        ...
        if (message.startsWith('GET ') || message.startsWith('POST ') || message.startsWith('PUT ') || message.startsWith('DELETE ') || message.startsWith('HEAD ')) {
            ...
            if (pathMatch) {
                ...
                // Check if this is a SmartFoxServer HTTP Box request (for polling mechanism)
                if (path.includes('HttpBox.do') || message.includes('sfsHttp=')) {
                    console.log('*** SMARTFOX HTTP BOX REQUEST DETECTED ***');
                    console.log('Processing SmartFoxServer HTTP polling request');

                    // Extract the sfsHttp parameter from the body (for POST) or query string (for GET)
                    let sfsHttpParam = null;
                    
                    // Extract from query string in the path
                    const queryMatch = path.match(/[?&]sfsHttp=([^&\s]*)/);
                    if (queryMatch) {
                        sfsHttpParam = decodeURIComponent(queryMatch[1]).replace(/\+/g, ' ');
                    }
                    
                    // If not found in query string, extract from body for POST requests
                    if (!sfsHttpParam) {
                        const bodyStart = message.indexOf('\r\n\r\n');
                        if (bodyStart !== -1) {
                            const body = message.substring(bodyStart + 4);
                            const bodyMatch = body.match(/sfsHttp=([^&\s]*)/);
                            if (bodyMatch) {
                                sfsHttpParam = decodeURIComponent(bodyMatch[1]).replace(/\+/g, ' ');
                            }
                        }
                    }

                    if (sfsHttpParam) {
                        console.log(`SmartFox HTTP parameter: ${sfsHttpParam}`);
                        
                        // Check if it's a poll request (format: #sessionId#poll)
                        if (sfsHttpParam.includes('#poll')) {
                            console.log('Processing HTTP poll request - sending empty response to keep connection alive');
                            
                            // Send empty response to acknowledge the poll (keeping connection alive)
                            // The client expects an empty response for poll requests
                            const pollResponse = 'HTTP/1.1 200 OK\r\n' +
                                               'Content-Type: text/plain\r\n' +
                                               'Content-Length: 0\r\n' +
                                               'Connection: keep-alive\r\n' +
                                               '\r\n';
                            socket.write(pollResponse);
                            console.log('Sent HTTP poll response');
                            return;
                        }
                        // Check if it's a handshake request (format: #connect)
                        else if (sfsHttpParam && sfsHttpParam.startsWith('#') && sfsHttpParam.includes('connect')) {
                            console.log('Processing HTTP handshake request');
                            
                            // Generate a fake session ID for the handshake
                            const fakeSessionId = Math.floor(Math.random() * 1000000).toString();
                            
                            // Send handshake response with session ID
                            const handshakeResponse = 'HTTP/1.1 200 OK\r\n' +
                                                     'Content-Type: text/plain\r\n' +
                                                     'Content-Length: ' + Buffer.byteLength('#' + fakeSessionId, 'utf8') + '\r\n' +
                                                     'Connection: keep-alive\r\n' +
                                                     '\r\n' +
                                                     '#' + fakeSessionId;
                            socket.write(handshakeResponse);
                            console.log(`Sent HTTP handshake response with session ID: ${fakeSessionId}`);
                            return;
                        }
                        // Check if it's a disconnect request
                        else if (sfsHttpParam.includes('disconnect')) {
                            console.log('Processing HTTP disconnect request');
                            
                            // Send disconnect acknowledgment
                            const disconnectResponse = 'HTTP/1.1 200 OK\r\n' +
                                                      'Content-Type: text/plain\r\n' +
                                                      'Content-Length: 0\r\n' +
                                                      'Connection: close\r\n' +
                                                      '\r\n';
                            socket.write(disconnectResponse);
                            console.log('Sent HTTP disconnect response');
                            return;
                        }
                        else {
                            console.log('Unknown SmartFox HTTP command:', sfsHttpParam);
                            
                            // Send empty response for unrecognized commands to keep connection alive
                            ...
                            console.log('Sent empty response for unrecognized command');
                            return;
                        }
                    } else {
                        console.log('Could not extract sfsHttp parameter from HTTP request');
                        
                        // Send empty response to keep connection alive even if we can't parse the request
                        const emptyResponse = 'HTTP/1.1 200 OK\r\n' +
                                            'Content-Type: text/plain\r\n' +
                                            'Content-Length: 0\r\n' +
                                            'Connection: keep-alive\r\n' +
                                            '\r\n';
                        socket.write(emptyResponse);
                        console.log('Sent empty response to maintain connection');
                        return;
                    }
                }
                // Handle manifest file requests by serving from the Manifests directory
                else if (path.includes('AppManifest.xml')) {
